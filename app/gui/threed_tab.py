...
from app.logic.controller import DicomController
...
class TileViewerTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        tb = QHBoxLayout(); tb.setSpacing(8); tb.setContentsMargins(0,0,0,16)
        ...
        tb.addWidget(self._btn_tiles); tb.addSpacing(10)
        tb.addWidget(self._btn_zoom_in); tb.addWidget(self._btn_zoom_out)
        tb.addWidget(self._btn_reset); tb.addStretch(); tb.addWidget(self._info)
        root.addLayout(tb)
        ...
        self._empty.setStyleSheet("background:#07050F; border-radius:12px; border:1px solid rgba(124,58,237,0.10);")
    ...
    def _on_display(self):
        ...
        if not frames: self._info.setText("No frames found"); return
        self._render(frames)
        self._info.setText(f"{len(frames)} slices")
        self._empty.setVisible(False); self._view.setVisible(True)
        QTimer.singleShot(0, self._fit_view)

    def _render(self, frames: List[np.ndarray]):
        self._scene.clear()
        cols = TILE_COLUMNS; tw = TILE_WIDTH; th = TILE_HEIGHT; pad = TILE_PADDING
        total = len(frames); rows = (total + cols - 1) // cols

        for i, arr in enumerate(frames):
            if arr.ndim != 2: arr = arr.squeeze()
            if arr.ndim != 2: continue
            qimg = QImage(arr.data, arr.shape[1], arr.shape[0], arr.shape[1],
    ...
    def _fit_view(self):
        if not self._scene.sceneRect().isValid(): return
        first_row = QRectF(0, 0, TILE_COLUMNS*(TILE_WIDTH+TILE_PADDING),
                           TILE_HEIGHT+TILE_PADDING+20)
