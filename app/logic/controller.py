...
class DicomController(QObject):
    ...
    def is_multiframe(self) -> bool:
        return self._model.is_multiframe

    @property
    def frame_count(self) -> int:
        return self._model.frame_count
    ...
    def get_all_tags(self) -> None:
        self.metadata_rows_ready.emit(self._model.get_all_tags())

    def search_tags(self, q) -> None:
        self.metadata_rows_ready.emit(self._model.search_tags(q))

    def get_patient_info(self) -> None:
        self.metadata_rows_ready.emit(self._tags_for_names(_PATIENT_TAGS))

    def get_study_info(self) -> None:
        self.metadata_rows_ready.emit(self._tags_for_names(_STUDY_TAGS))

    def get_modality_info(self) -> None:
        self.metadata_rows_ready.emit(self._tags_for_names(_SERIES_MODALITY_TAGS))

    def get_physician_info(self) -> None:
        self.metadata_rows_ready.emit(self._tags_for_names(_EQUIPMENT_PHYSICIAN_TAGS))

    def get_image_info(self) -> None:
        self.metadata_rows_ready.emit(self._tags_for_names(_IMAGE_TAGS))

    def get_pixel_data_info(self) -> None:
        self.metadata_rows_ready.emit(self._tags_for_names(_PIXEL_TAGS))
    ...
    def get_tile_frames(self) -> Tuple[List[np.ndarray], str]:
        if not self._model.is_loaded or not self._model.frames:
            return [], ""
        frames = [
            f for f in (prepare_frame_for_display(fr) for fr in self._model.frames) if f is not None
        ]
...
class DicomControllerExtended(DicomController):
    ...
    @property
    def wl_width(self) -> float:
        return self._wl_width

    @property
    def wl_center(self) -> float:
        return self._wl_center
    ...
    def is_playing(self) -> bool:
        return self._m2d_timer.isActive()

    def _redisplay(self) -> None:
        if not self.raw_hu_frames:
            return
    ...
    def _build_hu_frames(self) -> None:
        ...
        for frame in self._model.frames:
            ...
            if raw.ndim == 3:
                raw = (
                    (0.2989 * raw[:, :, 0] + 0.5870 * raw[:, :, 1] + 0.1140 * raw[:, :, 2])
                    if raw.shape[2] == 3
                    else raw.squeeze()
                )
    ...
    def pause_playback(self) -> None:
        if self._m2d_timer.isActive():
            ...
            self.status_message.emit(
                f"Paused  ·  frame {self._display_index+1} / {len(self.raw_hu_frames)}"
            )
    ...
    def load_series_folder(self, folder_path: str) -> None:
        try:
            ...
            self.status_message.emit(f"Folder scan failed: {exc}")
            return
        if not series:
            self.status_message.emit("No readable DICOM files found.")
            return
    ...
    def load_series_by_paths(self, file_paths: List[str]) -> None:
        ...
        self.status_message.emit(
            f"Series loaded  ·  {len(file_paths)} files  ·  " f"{self._model.frame_count} slices"
        )
        self._build_hu_frames()
