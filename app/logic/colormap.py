...
def _build_luts() -> Dict[str, np.ndarray]:
    ...
    bone_g = np.where(t < 0.375, t * 0.91,
             np.where(t < 0.75,  t * 0.91,       t * 0.91 + 0.05 * (t - 0.375)))
    bone_b = np.where(t < 0.375, t * 0.91 + 0.15 * t / 0.375,
                      t * 0.91)
    luts["Bone"] = (np.clip(np.stack([bone_r, bone_g, bone_b], axis=1), 0, 1) * 255).astype(np.uint8)

    # Jet — classic rainbow
    jet_r = np.where(t < 0.35, 0,
            np.where(t < 0.66, (t - 0.35) / 0.31,
            np.where(t < 0.89, 1.0,      (1.0 - (t - 0.89) / 0.11))))
    jet_g = np.where(t < 0.125, 0,
            np.where(t < 0.375, (t - 0.125) / 0.25,
            np.where(t < 0.64,  1.0,
            np.where(t < 0.91,  1.0 - (t - 0.64) / 0.27, 0))))
    jet_b = np.where(t < 0.11, 0.5 + t / 0.11 * 0.5,
            np.where(t < 0.34, 1.0,
            np.where(t < 0.65, 1.0 - (t - 0.34) / 0.31, 0)))
